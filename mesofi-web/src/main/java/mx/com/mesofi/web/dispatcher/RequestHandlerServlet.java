...
import mx.com.mesofi.web.response.FullWebResponse;
...
public class RequestHandlerServlet extends HttpServlet {
     ...
     * log4j.
     */
    private static Logger log = Logger.getLogger(RequestHandlerServlet.class);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        FullWebResponse response = handleWebRequest(POST.class, req);
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        FullWebResponse response = handleWebRequest(GET.class, req);
        handleWebResponse(response, req, resp);
    }
     ...
     * @return The response.
     */
    private FullWebResponse handleWebRequest(Class<? extends Annotation> requiredHttpMethod, HttpServletRequest req) {
        if (log.isDebugEnabled()) {
            log.debug("Requesting... " + req.getRequestURI());
        }
        FullWebResponse fullResponse = null;
        ...
        try {
            ...
            injectAddtionalServletConfig(springBean);
            ...
            if (!async.isEmpty()) {
                ...
                if (errorType.equals(ErrorType.BUSINESS)) {
                    ...
                    if (businessException.getFields() != null) {
                        response = new JsonResponse(new ErrorBusinessResponse(businessException.getFields()), false);
                    } else {
                        response = new JsonResponse(new ErrorBusinessResponse(businessException.getField()), false);
                    }
                } else {
    ...
    /**
     * In the current bean, injects more parameters if those exist.
     * 
     * @param springBean
     *            The current bean spring.
     */
    private void injectAddtionalServletConfig(Object springBean) {
        SimpleValidator.isNull(springBean, "The spring bean should not be null");
        for (Field field : springBean.getClass().getDeclaredFields()) {
            if (field.isAnnotationPresent(Inject.class)) {
                if (field.getType().isAssignableFrom(ServletContext.class)) {
                    ...
                    try {
                        field.set(springBean, getServletContext());
     ...
     *             In case an exception occurs.
     */
    private void handleWebResponse(FullWebResponse response, HttpServletRequest req, HttpServletResponse resp)
            throws IOException, ServletException {
        // the content type
        resp.setContentType(response.getWebResponse().getMediaType().toString());
        if (response.getWebResponse() instanceof StreamResponse) {
            StreamResponse streamResponse = (StreamResponse) response.getWebResponse();
            resp.setHeader("Content-Length", String.valueOf(streamResponse.getFileSize()));
            resp.setHeader("Content-Disposition", "attachment;filename=" + streamResponse.getFileName());
            // process an stream
            streamResponse.getResponse(resp.getOutputStream());
        } else {
            if (resp.getContentType().equals(MediaType.TEXT_HTML)) {
                // forwards the page to another resource view.
                RequestDispatcher dispatcher = req.getRequestDispatcher(createResponseUrl(response.getContent()));
        ...
        if (log.isDebugEnabled()) {
            log.debug("[" + response.getWebResponse().getMediaType()
                    + "] The response has been returned successfully!!!");
        }
    }
     ...
     * The first element contains the content type, the second contains the
     * actual value of the response and the last one contains whole WebResponse.
     * 
     * @param webResponse
     *            Web response.
     * @param accept
     ...
     */
    private FullWebResponse createResponse(WebResponse webResponse, String accept) {
        FullWebResponse fullWebResponse = new FullWebResponse();
        try {
            if (webResponse instanceof StreamResponse) {
                // it does nothing, avoid to invoke the getResponse method on
                // this type.
            } else if (webResponse instanceof HeaderResponse) {
                webResponse = ((HeaderResponse) webResponse).processBasedOnHeader(accept);
                fullWebResponse.setContent(webResponse.getResponse());
            } else {
                fullWebResponse.setContent(webResponse.getResponse());
            }
            fullWebResponse.setWebResponse(webResponse);
